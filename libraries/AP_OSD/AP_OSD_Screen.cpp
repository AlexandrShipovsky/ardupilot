...
void AP_OSD_Screen::draw_bat_volt(uint8_t instance, VoltageType type, uint8_t x, uint8_t y)
{
    ...
    case VoltageType::AVG_CELL:
    {
        ...
        else if (osd->cell_count < 0)
        { // user must decide on autodetect cell count or manually entered to display this panel since default is -1
            backend->write(x, y, false, "%c---%c", SYMBOL(SYM_BATT_FULL) + p, SYMBOL(SYM_VOLT));
            return;
        }
        else
        { // use autodetected cell count
...
#define CROSS 0xF0
#define max_x (30 * 12 - 1) // 1 ..29
#define max_y (16 * 18 - 1) // 1 ..15
// uint32_t last_log_ms=0;
void AP_OSD_Screen::draw_followme(uint8_t x, uint8_t y)
{
    AP_OSD *osdobj = AP::osd();
    const char str[5] = "3AXB";
    int src_x,src_y;

    if (osd->status_followme)
    {
        src_x = (osdobj->x_followme + 1) * max_x / 2;
        src_y = (osdobj->y_followme + 1) * max_y / 2;
    ...
    int pos_x = src_x / 12;
    int offset_x = (src_x - pos_x * 12) / 4;
    int pos_y = src_y / 18;
    int offset_y = (src_y - pos_y * 18) / 6;
    int cross = CROSS + offset_x + offset_y * 3;
    backend->write(pos_x, pos_y, false, "%c", cross);

    if ((pos_x > 0) && (pos_y > 0))
        backend->write(pos_x - 1, pos_y - 1, false, "%c", 0xF9); // SYMBOL(SYM_X1)
    if ((pos_x < 28) && (pos_y > 0))
        backend->write(pos_x + 1, pos_y - 1, false, "%c", 0xFA); // SYMBOL(SYM_X2)
    if ((pos_x > 0) && (pos_y < 15))
        backend->write(pos_x - 1, pos_y + 1, false, "%c", 0xFB); // SYMBOL(SYM_X3)
    if ((pos_x < 28) && (pos_y < 15))
        backend->write(pos_x + 1, pos_y + 1, false, "%c", 0xFC); // SYMBOL(SYM_X4)
}
